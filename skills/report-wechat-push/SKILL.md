...
当需要一次性推送多只基金报告时，可使用 `fund-weekly-review` skill 中提供的批量推送脚本：

```bash
cd D:/OPC-skill/skills/fund-weekly-review
python batch_wechat_push_v2.py
```

该脚本会：
1. 自动读取 `fund_source.json` 中的基金数据
2. 对每只基金发送 Markdown 速览消息（含净值、周收益、成立以来收益）
3. 上传并发送对应的 PDF 附件
4. 每只基金间隔 2 秒（企微频率限制）
...
| 版本 | 日期 | 变更内容 |
|------|------|---------|
| v2.1 | 2026-07-20 | 补充批量推送脚本说明：`fund-weekly-review/batch_wechat_push_v2.py` 支持10只基金自动轮询推送，每只间隔2秒；消息格式固化为 `[咖啡]{名称}周报速览` + 三行数据 + PDF附件 |
