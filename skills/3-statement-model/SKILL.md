...
## China Market Financial Data Extraction

If the template requires pulling data from public company financial statements (年报/季报), use domestic data sources:

**Primary Sources:**
1. **AKShare** - `stock_financial_report_sina(stock="600519", symbol="利润表")` for income statement
   - `stock_financial_report_sina(stock="600519", symbol="资产负债表")` for balance sheet
   - `stock_financial_report_sina(stock="600519", symbol="现金流量表")` for cash flow statement
   - `stock_yjbb_em(date="20241231")` for batch financial indicators
2. **Tushare Pro** - `pro.income(ts_code='600519.SH')`, `pro.balancesheet()`, `pro.cashflow()`
3. **巨潮资讯网** - Authoritative source via `ak.stock_zh_a_disclosure_report_cninfo()`

**For historical reference only (US companies):** If populating a model for a US-listed company, see [references/sec-filings.md](references/sec-filings.md) for SEC EDGAR extraction guidance.

For detailed data retrieval code templates, reference the `china-market-data` skill.
