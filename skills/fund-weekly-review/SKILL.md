...
`fund-weekly-review` 提供统一入口脚本 `run.py`，自动完成数据爬取 → 生成中间数据 → 生成PDF/HTML → 更新首页 → Git推送全流程：

```bash
cd D:/OPC-skill/skills/fund-weekly-review

# 单只基金
python run.py --code 002692 --period 2026-07-13:2026-07-17

# 多只基金批量
python run.py --codes 002692,100055,014736 --period 2026-07-13:2026-07-17

# 跳过数据爬取（复用现有数据）
python run.py --code 002692 --period 2026-07-13:2026-07-17 --skip-fetch

# 只生成本地报告，不上传部署
python run.py --code 002692 --period 2026-07-13:2026-07-17 --no-deploy
```

参数说明：
| 参数 | 必填 | 说明 |
|------|------|------|
| `--code` / `-c` | 单只时必填 | 基金代码，如 `002692` |
| `--codes` | 多只时必填 | 逗号分隔的基金代码列表，如 `002692,100055` |
| `--period` / `-p` | 必填 | 报告周期，格式 `YYYY-MM-DD:YYYY-MM-DD` |
| `--no-deploy` | 否 | 跳过 `git push`，只生成本地报告 |
| `--skip-fetch` | 否 | 跳过天天基金网数据爬取，复用现有数据 |
...
| 版本 | 日期 | 变更内容 |
|------|------|---------|
| v3.4 | 2026-07-20 | 新增PDF水印：`add_watermark`函数每页添加斜体半透明水印"内部资料，严禁转发"（SimHei 48pt，透明度6%，旋转-35°）；HTML产品概况改为3x3卡片网格布局（overview-card），正红负绿着色；修复fetch_data_v4日期提取Bug（去掉fix_date尾部多余`)`符号） |
| v3.3 | 2026-07-17 | 固化产品概况数据结构：`overview`固定9项，2列垂直布局，列宽`[40mm, 140mm]`；`colored_pct`正红`#dc2626`负绿`#16a34a`；`最新净值`纯数字不含日期后缀 |
| v3.2 | 2026-07-17 | 新增`enrich_pdf_fields`自动补全15个PDF关键字段；`generate_pdfs_batch.py`改为读取`funds_data_pdf.json` |
| v3.1 | 2026-07-17 | `generate_htmls.py`去硬编码：sec6~sec13数据变量注入；`funds_data.json`数据结构明确为字典格式 |
| v3.0 | 2026-07-10 | 固化PDF/HTML模板样式；新增HTML 14模块结构；差异化下载策略（PC下载/移动预览） |
| v2.0 | 2026-06-20 | 新增量化型分析框架，支持主动权益与量化双框架切换 |
| v1.0 | 2026-06-10 | 初始版本，支持Markdown + PDF输出 |
