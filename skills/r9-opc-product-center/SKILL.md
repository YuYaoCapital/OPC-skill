...
| 周度回顾 | `fund-weekly-review` | 持营基金周度回顾报告（PDF） |
| 融合报告 | `fund-weekly-hybrid` | 周度回顾+基金经理横纵分析融合报告（PDF） |
...
| Skill | 用途 | 输出格式 | 面向对象 |
|-------|------|----------|----------|
| `portfolio-week-companion` | **面向客户的**周度陪伴报告 | 单页 HTML 网页 | 客户（投后陪伴） |
| `fund-weekly-review` | **面向理财经理的**纯周度回顾报告 | Markdown / PDF（红色主调） | 理财经理（专业分析） |
| `fund-weekly-hybrid` | **融合版**：周度回顾 + 基金经理横纵分析 | Markdown / PDF（红色主调） | 理财经理（深度评价） |

> **选择指南**：
> - 客户需要看报告 → `portfolio-week-companion`（HTML 网页，实时数据，沟通话术）
> - 理财经理需要专业回顾 → `fund-weekly-review`（PDF，持仓归因，主动权益/量化双框架）
> - 理财经理需要深度评价 + 回顾 → `fund-weekly-hybrid`（PDF，在 review 基础上增加基金经理横纵分析法）
